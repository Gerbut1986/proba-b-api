...
namespace BinanceOptionsApp.Controls
{
    internal class IntegerTextBox : TextBox
    {
        public IntegerTextBox()
        {
            ...
            TextChanged += new TextChangedEventHandler(IntegerTextBox_TextChanged);
            PreviewKeyDown += new KeyEventHandler(IntegerTextBox_PreviewKeyDown);
            PreviewTextInput += new TextCompositionEventHandler(IntegerTextBox_PreviewTextInput);
            MouseDoubleClick += new MouseButtonEventHandler(IntegerTextBox_MouseDoubleClick);

            LostKeyboardFocus += new KeyboardFocusChangedEventHandler(IntegerTextBox_LostKeyboardFocus);

            OnNewDependencyPropertyChanged();
        }

        void IntegerTextBox_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
        ...
        void IntegerTextBox_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Space)
            {
                e.Handled = true;
        ...
        void IntegerTextBox_LostKeyboardFocus(object sender, KeyboardFocusChangedEventArgs e)
        {
        ...
        void IntegerTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            string txt = Text;
            ...
            int value;
            if (StringToValue(newText, out value)) // если удается преобразовать в int - установить значение и текст
            {
                freeze_text = newText;
                if (Value != value)
                {
        ...
        bool StringToValue(string str, out int value)
        {
            ...
            int v = 0;
            bool result = int.TryParse(str, NumberStyles.Any, CultureInfo.InvariantCulture, out v);
