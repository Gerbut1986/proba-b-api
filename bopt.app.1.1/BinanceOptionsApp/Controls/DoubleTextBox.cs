...
namespace BinanceOptionsApp.Controls
{
    internal class DoubleTextBox : TextBox
    {
        ...
        void DoubleTextBox_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
        ...
        {
            DoubleTextBox control = d as DoubleTextBox;
        ...
        void DoubleTextBox_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Space)
            {
                e.Handled = true;
        ...
        void DoubleTextBox_PreviewTextInput(object sender, TextCompositionEventArgs e)
        {
        ...
        void DoubleTextBox_LostKeyboardFocus(object sender, KeyboardFocusChangedEventArgs e)
        {
        ...
        void DoubleTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            double value;
            if (StringToValue(newText, out value)) // если удается преобразовать в double - установить значение и текст
            {
                freeze_text = newText;
                if (Value != value)
                {
        ...
        bool StringToValue(string str, out double value)
        {
            ...
            double v = 0;
            bool result = double.TryParse(str, NumberStyles.Any, CultureInfo.InvariantCulture, out v);

            value = v * Math.Pow(10, Power);
            return result;
        }
        string ValueToString(double value)
        {
            double mul = Math.Pow(10, Power);
            value /= mul;
