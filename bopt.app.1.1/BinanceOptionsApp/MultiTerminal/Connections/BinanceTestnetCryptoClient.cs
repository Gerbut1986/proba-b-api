...
{
    internal class BinanceTestnetCryptoClient : BaseCryptoClient
    {
        ...
        public BinanceTestnetCryptoClient(IConnectorLogger logger, ManualResetEvent cancelToken, TestnetConnectionModel model) : base(logger, cancelToken, model)
        {
        }
        ...
        internal async Task<decimal> GetBalance(string Asset)
        {
            ...
            try
            {
                ...
                {
                    IEnumerable<BalanceTestnet> Balances = await CallAsync<IEnumerable<BalanceTestnet>>(EndPoints.BalanceT, $"recvWindow={recvWindow}");
                    balance = Balances.Single(ua => ua.Asset == Asset).Balance;
                }
        ...
        {
            WebSocket wsDepth = new WebSocket($"wss://fstream.binancefuture.com/ws/{data.quote.Symbol.ToLower()}@depth");
        ...
        private async void StartUserDataStream()
        {
            try
            {
                BinanceEndPoint ep = (AccountTradeType == AccountTradeType.SPOT) ? EndPoints.SpotCreateListenKeyFuture : EndPoints.MarginCreateListenKeyFuture;
        ...
        public async Task<T> CallAsync<T>(BinanceEndPoint endpoint, string parameters = null, bool firstCall = true)
        {
            ...
            if (endpoint.isSigned)
            {
                ...
                queryStringBuilder.Append(parameters).Append("&").Append("timestamp=").Append(now);
                string signature = SignatureHelper.Sign(queryStringBuilder.ToString(), Secret);
                queryStringBuilder.Append("&signature=").Append(signature);
                finalEndpoint = endpoint.Value + "?" + queryStringBuilder.ToString();
            }

            string urlPath = "https://testnet.binancefuture.com" + finalEndpoint;
            ...
            if (firstCall && eCode == -1021)
            {
                ...
                BinanceServerTime sTime = await CallAsync<BinanceServerTime>(EndPoints.CheckServerTimeT);
